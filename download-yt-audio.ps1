...
<#  Calls the yt-dlp with options specified below.
    ...
    --format bestaudio[ext=m4a]:
        automatically selects the best audio format possible.
    --output "%(uploader)s - %(title)s - %(upload_date)s.%(ext)s":
        Once the download process has finished, yt-dlp will rename the file
        according to the specified format.
    --no-part:
        yt-dlp will not download the audio into multiple parts.
    --add-metadata:
        metadata will be embedded to the final downloaded audio.
    --write-description:
        writes the audio's description to a separate text file.
#>
